...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct ImageHookConfig {
    command: Vec<String>,
    #[serde(default)]
    environment: Vec<String>,
}

impl ImageHookConfig {
    fn payload(&self, region: &str, hook: &str) -> Result<String, ClankerError> {
        let (command, args) = self.command.split_first().ok_or_else(|| {
            ClankerError::InvalidConfig(format!(
                "microvm.image.hooks.{hook}.command must not be empty"
            ))
        })?;
        let environment = parse_key_values(
            &self.environment,
            &format!("{hook} environment variable"),
            false,
        )?;
        build_image_hook_payload(command, args, environment, region, hook)
    }
}
    ...
    fn try_from(file: ProjectFile) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            push: PushSettings {
                ...
                validate_timeout_seconds: hook_seconds(
                    image.hooks.validate_timeout,
                    "validate-timeout",
                )?,
    ...
    pub(crate) fn image_spec(
        ...
        Ok(ImageSpec {
            ...
            configuration: ImageConfiguration {
                ...
                hooks: image_hooks(settings, self.validate.is_some()),
    ...
    fn image_environment(&self) -> Result<HashMap<String, String>, ClankerError> {
        ...
        if let Some(ready) = &self.ready {
            environment.insert(
                ...
                ready.payload(&self.aws.region, "ready")?,
            );
        }
        if let Some(validate) = &self.validate {
            environment.insert(
                "CLANKERVM_VALIDATE_HOOK_PAYLOAD".into(),
                validate.payload(&self.aws.region, "validate")?,
            );
        }
        Ok(environment)
    }
...
fn image_hooks(settings: &PushSettings, validate: bool) -> Hooks {
    Hooks::builder()
        .port(settings.port())
        .microvm_image_hooks(
            MicrovmImageHooks::builder()
                ...
                .set_validate_timeout_in_seconds(
                    validate.then(|| settings.validate_timeout_seconds()),
                )
                .build(),
        )
        .microvm_hooks(
...
"#;
    ...
    #[test]
    fn image_commands_use_run_shaped_settings_and_the_resolved_region() {
        for (hook, variable) in [
            ("ready", "CLANKERVM_READY_HOOK_PAYLOAD"),
            ("validate", "CLANKERVM_VALIDATE_HOOK_PAYLOAD"),
        ] {
            let text = format!(
                "{CONFIG}\n[microvm.image.hooks.{hook}]\ncommand = ['echo', 'hello world']\nenvironment = ['EMPTY=', 'VALUE=a=b', 'AWS_REGION=ignored']"
            );
            ...
            let payload: Value = serde_json::from_str(&environment[variable]).unwrap();
    ...
    #[test]
    fn image_commands_reject_invalid_or_oversized_configuration() {
        for fields in [
    ...
    #[test]
    fn validate_timeout_defaults_and_limits() {
        assert_eq!(PushSettings::default().validate_timeout_seconds(), 300);
        for seconds in [0, 3601] {
            assert!(
                PushSettings {
                    validate_timeout_seconds: Some(seconds),
                    ..PushSettings::default()
                }
                .validate()
                .is_err()
